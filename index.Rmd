...
dat_for_clust <- IO91_48hrs_dat_bands %>%
  filter(MHz.y == "14.0956")
nrow(dat_for_clust)
```


This kinda works for 14 MHz (20m):

* Three clusters
* First cluster mean is left free
* Second mean $a$
* Third $2a$
* SD constant for all clusters

```{r}
cluster_n = 3
means <- c(NA, paste0(1:(cluster_n-1), "n"))
sds   <- c(rep("b", cluster_n))
...
```{r fig.height=3, fig.width=6, dpi = 300}
plot(ecdf(dat_for_clust$distance))
```
